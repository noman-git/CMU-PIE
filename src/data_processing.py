...
def pre_process_data(data, num_labels, train_rows, test_rows):
    truncated_df, labels = create_df(data, num_labels) #create the df
    normalized_data_df = normalize_data(truncated_df) #normalize the data
    normalized_data_df['label'] = labels
    return create_data_splits(normalized_data_df, num_labels, train_rows, test_rows, num_splits) #create the data splits
...
def prepare_data(data, labels_list, train_test_pairs):
    train_test_dfs = [pre_process_data(data, num_labels, train_rows, test_rows) for num_labels in labels_list for train_rows, test_rows in train_test_pairs]
